...
import { MediaStreamWithBoost } from '../types/mediaStreamWithBoost';
import { ClientParams } from '../types/ClientParams';
import { Result } from '../types/TranscriptEvent';
...
export default function streamAudioToWebSocket(
  userMediaStream: MediaStreamWithBoost,
  onChunk: (chunk: Result) => void,
  ...
  websocket.onopen = function () {
    pcmStream.onData((audio) => {
      ...
      if (websocket.readyState === websocket.OPEN) websocket.send(binary);
    });
  };
